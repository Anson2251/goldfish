...
class ActivationStatsProbe:
    ...
    def collect(self, context: HookContext) -> Mapping[str, Any] | None:
        ...
        try:
            with torch.no_grad():
                for point, modules in resolved:
                    for _, module in modules:
                        accumulator = _Accumulator(point)
                        accumulators[id(module)] = accumulator
                        handles.append(module.register_forward_hook(_make_hook(accumulator)))
        ...
        for point, modules in resolved:
            for path, module in modules:
                entries.append(accumulators[id(module)].finalize(path))
...
def _make_hook(accumulator: _Accumulator):
    def hook(module: nn.Module, inputs: tuple[Any, ...], outputs: Any) -> None:
        if accumulator.point.quantity is not None:
            accumulator.add_quantity(module, inputs, outputs)
        else:
            for spec in accumulator.point.tensors:
                tensor = _resolve_tensor(spec.name, module, inputs, outputs)
                accumulator.add_tensor(spec, tensor)

    return hook


def _resolve_tensor(name: str, module: nn.Module, inputs: tuple[Any, ...], outputs: Any) -> Tensor:
    if name == "input":
    ...
    if name in ("hidden", "cell"):
        if not isinstance(outputs, tuple) or len(outputs) < 2 or not isinstance(outputs[1], tuple):
            raise ValueError(f"module {type(module).__name__} does not return the LSTM contract for {name!r}")
    ...
    if name not in diagnostics:
        raise ValueError(f"diagnostics of {type(module).__name__} has no tensor {name!r}")
...
class _Accumulator:
    ...
    def add_tensor(self, spec: _TensorSpec, tensor: Tensor) -> None:
        for stat in spec.stats:
            values = _per_element(tensor, stat)
            if spec.reduce == "per_head":
                if values.ndim != 3:
                    raise ValueError(
                        f"per_head reduce requires a [B, T, N, D] tensor, got shape {tuple(tensor.shape)}"
                    )
                self._per_head[f"{spec.name}.{stat}"].append(values)
            else:
                self._values[f"{spec.name}.{stat}"].append(values.flatten())

    def add_quantity(self, module: nn.Module, inputs: tuple[Any, ...], outputs: Any) -> None:
        ...
        if quantity == "io-stats":
            ...
            self._add_stat("input_norm", input_tensor, "norm")
            self._add_stat("output_norm", output_tensor, "norm")
            self._add_values("displacement_ratio", (output_tensor - input_tensor).norm(dim=-1) / input_tensor.norm(dim=-1))
        elif quantity == "mixing-displacement":
            diagnostics = module.diagnostics(_as_tensor(inputs[0]))  # type: ignore[attr-defined]
            input_tensor = diagnostics["input"]
            output_tensor = diagnostics["output"]
            displacement = (output_tensor - input_tensor).norm(dim=-1) / input_tensor.norm(dim=-1)
            self._add_stat("input_norm", input_tensor, "norm")
            self._add_values("displacement_ratio", displacement)
            self._add_per_head("displacement_ratio_per_head", displacement)
            ...
            self._add_per_head("injection_ratio_per_receiver", gated.norm(dim=-1) / states_norm)
            self._add_per_head("decoded_ratio_per_receiver", decoded.norm(dim=-1) / states_norm)
    ...
    def _add_values(self, key: str, values: Tensor) -> None:
        self._values[key].append(values.flatten())

    def _add_per_head(self, key: str, values: Tensor) -> None:
        if values.ndim != 3:
            raise ValueError(f"per_head reduce requires [B, T, N] values, got shape {tuple(values.shape)}")
        self._per_head[key].append(values)
        ...
        if self.point.quantity is not None:
            ...
            for key, values in per_head.items():
                if key.endswith("_per_receiver"):
                    entry[key.removesuffix("_per_receiver") + "_mean"] = float(torch.tensor(values).mean())
            return entry
