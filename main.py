"""Prepare and train character language models from versioned dataset roots."""
...
def _default_prompt(dataset_root: Path, manifest: Any) -> str:
    ...
    for entry in entries:
        relative_path = entry if isinstance(entry, str) else entry.get("path") if isinstance(entry, dict) else None
        if not isinstance(relative_path, str):
            raise ValueError("Manifest train split file entries must be paths or mappings with a path.")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    prepare = commands.add_parser("prepare", help="Validate a dataset manifest and prepare its text artifacts.")
    prepare.add_argument("dataset_root", type=Path, help="Directory containing manifest.yaml and dataset splits.")

    train = commands.add_parser("train", help="Train from a prepared, locked dataset.")
    train.add_argument("dataset_root", type=Path, help="Prepared dataset directory containing manifest.yaml.")
    train.add_argument("--runs-dir", type=Path, default=Path("runs"), help="Experiment run directory (default: runs).")
    train.add_argument("--name", help="Optional name appended to a new run directory.")
    train.add_argument("--resume", type=Path, metavar="RUN_DIR", help="Strictly resume an existing run from latest.pt.")
    train.add_argument("--sequence-length", type=int, default=64, help="Tokens in each training window (default: 64).")
    train.add_argument("--batch-size", type=int, default=32, help="Training and validation batch size (default: 32).")
    train.add_argument("--epochs", type=int, default=5, help="Epochs to train; on resume these are additional epochs (default: 5).")
    train.add_argument("--embedding-dim", type=int, default=64, help="Character embedding width (default: 64).")
    train.add_argument("--hidden-dim", type=int, default=128, help="Recurrent hidden width (default: 128).")
    train.add_argument("--num-layers", type=int, default=1, help="Recurrent layer count (default: 1).")
    train.add_argument("--dropout", type=float, default=0.0, help="Recurrent dropout (default: 0).")
    train.add_argument("--model", choices=model_registry.names("language"), default="gru", help="Language-model architecture (default: gru).")
    train.add_argument("--learning-rate", "--lr", type=float, default=1e-3, help="Optimizer learning rate (default: 0.001).")
    train.add_argument("--optimizer", choices=("adamw", "adam", "sgd"), default="adamw")
    train.add_argument("--weight-decay", type=float, help="Optimizer weight decay (optimizer default when omitted).")
    train.add_argument("--momentum", type=float, help="SGD momentum.")
    train.add_argument("--scheduler", choices=("none", "cosine", "step", "exponential", "plateau"), default="none")
    train.add_argument("--scheduler-step-timing", choices=("batch", "epoch", "validation"))
    train.add_argument("--scheduler-t-max", type=int, help="Cosine scheduler T_max.")
    train.add_argument("--scheduler-eta-min", type=float, help="Cosine scheduler minimum learning rate.")
    train.add_argument("--scheduler-step-size", type=int, help="Step scheduler interval.")
    train.add_argument("--scheduler-gamma", type=float, help="Step/exponential scheduler gamma.")
    train.add_argument("--scheduler-factor", type=float, help="Plateau scheduler factor.")
    train.add_argument("--scheduler-patience", type=int, help="Plateau scheduler patience.")
    train.add_argument("--gradient-clip-norm", type=float, help="Maximum gradient norm.")
    train.add_argument("--sample-frequency", type=int, default=1, help="Write a sample every N epochs (default: 1).")
    train.add_argument("--checkpoint-frequency", type=int, help="Write an epoch-NNNN checkpoint every N epochs.")
    train.add_argument("--checkpoint-path", "--output-path", type=Path, help="Optional legacy checkpoint destination.")
    train.add_argument("--prompt", help="Generation prompt; defaults to the first training document.")
    train.add_argument("--max-new-tokens", type=int, default=100, help="Maximum tokens to generate (default: 100).")
    train.add_argument("--seed", type=int, default=0, help="Torch random seed (default: 0).")
    return parser
...
def _prepare(dataset_root: Path) -> int:
    prepare_text_dataset = _future_api("goldfish.data.text", "prepare_text_dataset")
    write_dataset_lock = _future_api("goldfish.data.validation", "write_dataset_lock")
    write_tokenizer_lock = _future_api("goldfish.data.validation", "write_tokenizer_lock")
    manifest = _validate_manifest(dataset_root)
    prepare_text_dataset(dataset_root, manifest)
    write_dataset_lock(dataset_root, manifest)
    write_tokenizer_lock(dataset_root, manifest)
...
def _resolved_config(args: argparse.Namespace, manifest: Mapping[str, Any], vocab_size: int, prompt: str) -> dict[str, Any]:
    ...
    config = {
        ...
        "task": {"name": "causal_language_model"},
        **resolved.to_mapping(),
        "training": {"epochs": args.epochs, "gradient_clip_norm": args.gradient_clip_norm},
...
def _write_sample(run: ExperimentRun, *, epoch: int | None, model: Any, tokenizer: TextTokenizer, config: Mapping[str, Any]) -> str:
    ...
    text = cast(str, generate_text(model, tokenizer, str(generation["prompt"]), max_new_tokens=int(generation["max_new_tokens"])))
...
def _train(args: argparse.Namespace) -> int:
    ...
    if args.resume is None:
        run = ExperimentRun.create(args.runs_dir, name=args.name, config=config, data=data, environment=collect_environment(repository=Path(__file__).parent))
    else:
    ...
    def on_epoch_end(result: Any) -> None:
        ...
        if (result.epoch + 1) % frequency == 0:
            _write_sample(run, epoch=result.epoch, model=model, tokenizer=cast(TextTokenizer, data_module.tokenizer), config=config)

    trainer = Trainer(model, cast(Task[Any], CausalLanguageModelTask()), optimizer, scheduler=cast(Any, scheduler), scheduler_step_timing=resolved_training.scheduler.step_timing, scheduler_metric=getattr(resolved_training.scheduler, "monitor", None), gradient_clip_norm=cast(Mapping[str, Any], config["training"])["gradient_clip_norm"], on_epoch_end=on_epoch_end, progress=False)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "prepare":
        return _prepare(args.dataset_root)
